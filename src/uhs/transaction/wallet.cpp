...
namespace cbdc {
    ...
    auto transaction::wallet::mint_new_coins(const size_t n_outputs,
        ...
        const pubkey_t pubkey = generate_minter_key();

        // First, generate the outputs in order to create a txid
        ...
        privkey_t seckey = m_keys.at(pubkey);

        // Sign for each output
        for(size_t i = 0; i < n_outputs; i++) {
    ...
    auto transaction::wallet::generate_key() -> pubkey_t {
        // Unique lock on m_keys, m_keygen and m_keygen_dist
        {
            ...
            // NOTE: changed to start at index 1. 0 is reserved for the
            // minter's key
